...
void idAudioHardwareALSA::Write( bool flushing ) {
	...
	int byteOffset = ( MIXBUFFER_SAMPLES - m_remainingFrames ) * m_channels * 2;
	byte *p_writeBuffer = reinterpret_cast<byte *>( m_buffer ) + byteOffset;
	snd_pcm_sframes_t frames = id_snd_pcm_writei( m_pcm_handle, p_writeBuffer, m_remainingFrames );
	if ( frames < 0 ) {
		if ( frames != -EAGAIN ) {
